...
class face_check:
	def __init__(self):
		...
		self.personPose_pub = rospy.Publisher('person_pose', PoseStamped, queue_size=1)
		...
		depth_sub = message_filters.Subscriber('/camera/depth/image_rect_raw/decompressed', Image)
		img_sub = message_filters.Subscriber('/camera/color/image_raw/decompressed', Image)
	...
	def depth_rgb_callback(self, depth_msg, img_msg):
		...
		if not self.img_processing:
			...
			try:
				camera_transform = self.tf_buffer.lookup_transform('odom', 'camera_link', depth_msg.header.stamp)
			except:
				self.img_processing = False
				return
