...
class clothes_pattern:
	...
	def depth_img_callback(self, depth_msg, img_msg):
		...
		if not self.img_processing:
			...
			self.img_openpose_pub.publish(img_msg)
			...
			# Open pose
			try:
				human_list_msg = rospy.wait_for_message('/openpose_ros/human_list', OpenPoseHumanList, timeout=0.5)
			except:
				self.img_processing = False
				return
